...
cleanup_on_inactivity_test() ->
    ...
    timer:sleep(500),
    {Active, LastActivity, LastEviction, _} = gblob_server:status(Server),
...
set_active_on_action_after_cleanup_test() ->
    ...
    timer:sleep(500),
    {Active, LastActivity, _, _} = gblob_server:status(Server),
    ?assertEqual(false, Active),
    ?assertEqual(true, LastActivity > 0),
    Item = gblob_server:get(Server, 42),
    {Active1, LastActivity1, _, _} = gblob_server:status(Server),

...
write_4_close_read_all_raw(#sblob{path=Path, name=ChunkName}=Sblob) ->
    ...
    Count = 4,
    SeqNum = 1,
    {ok, [E1, E2, E3, E4], RFirstSN, RSeqNum, RCount} = sblob_util:seqread(Path, ChunkName, SeqNum, Count, []),
...
write_4_close_read_first_2_raw(#sblob{path=Path, name=ChunkName}=Sblob) ->
    ...
    Count = 2,
    SeqNum = 1,
    {ok, [E1, E2], RFirstSN, RSeqNum, RCount} = sblob_util:seqread(Path, ChunkName, SeqNum, Count, []),
...
write_4_close_read_middle_2_raw(#sblob{path=Path, name=ChunkName}=Sblob) ->
    ...
    Count = 2,
    SeqNum = 2,
    {ok, [E2, E3], RFirstSN, RSeqNum, RCount} = sblob_util:seqread(Path, ChunkName, SeqNum, Count, []),
...
write_4_close_read_last_2_raw(#sblob{path=Path, name=ChunkName}=Sblob) ->
    ...
    Count = 4,
    SeqNum = 3,
    {ok, [E3, E4], RFirstSN, RSeqNum, RCount} = sblob_util:seqread(Path, ChunkName, SeqNum, Count, []),
    [?_assertEqual(RSeqNum, 4),
     ?_assertEqual(RFirstSN, 1),
     ?_assertEqual(RCount, 2),
     assert_entry(E3, <<"asd 2">>, 3),
